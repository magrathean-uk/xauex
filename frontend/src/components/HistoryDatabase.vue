<template>
  ...
  >
    ...
    <div v-if="projects.length > 0" class="history-summary-list">
      <button
        v-for="project in projects.slice(0, 6)"
        :key="`summary-${project.simulation_id}`"
        class="history-summary-item"
        @click="navigateToProject(project)"
      >
        <div class="summary-top">
          <span class="summary-id">{{ formatSimulationId(project.simulation_id) }}</span>
          <span class="summary-time">{{ formatDate(project.created_at) }} {{ formatTime(project.created_at) }}</span>
        </div>
        <div class="summary-title">{{ getSimulationTitle(project.simulation_requirement) }}</div>
        <div class="summary-meta">
          <span class="summary-rounds">{{ formatRounds(project) }}</span>
          <span class="summary-report" :class="{ ready: project.report_id }">
            {{ project.report_id ? 'REPORT READY' : 'NO REPORT' }}
          </span>
        </div>
      </button>
    </div>
...
.history-summary-item:hover {
  ...
  border-color: rgba(242, 98, 60, 0.35);

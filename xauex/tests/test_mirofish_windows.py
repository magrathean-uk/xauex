...
def _build_orchestrator() -> object:
    ...
    orchestrator.config = SimpleNamespace(
        mirofish_entry_timezone="Europe/London",
    ...
    orchestrator.risk_state.mirofish_signal_runs_london = []
    orchestrator._recent_h1_closes = [2350.0] * 20
    orchestrator._trade_entries_on_chart = []
    orchestrator._last_mirofish_signal_id_by_slot = {}
    orchestrator._last_mirofish_signal_seen_utc = {}
    orchestrator._mirofish_close_requested = {}
    return orchestrator
...
def test_entry_slot_selection_includes_morning_and_midday_windows():
    ...
    assert orch._mirofish_entry_slot(_dt("2026-04-07T07:59")) is None
    assert orch._mirofish_entry_slot(_dt("2026-04-07T08:02")) == "MORNING"
    assert orch._mirofish_entry_slot(_dt("2026-04-07T08:06")) is None

    assert orch._mirofish_entry_slot(_dt("2026-04-07T11:27")) is None
    assert orch._mirofish_entry_slot(_dt("2026-04-07T11:32")) == "MIDDAY"
    assert orch._mirofish_entry_slot(_dt("2026-04-07T11:40")) is None


def test_weekends_are_not_tradable():
    orch = _build_orchestrator()
    assert orch._mirofish_entry_slot(_dt("2026-04-04T08:02")) is None
...
def test_slot_usage_is_tracked_and_persists_until_next_slot():
    ...
    orch._record_mirofish_signal_run(
        slot="MORNING",
        signal_id="run-1",
        action="HOLD",
        signal_time=now,
...
def test_second_slot_is_still_available_if_morning_slot_was_used():
    ...
    assert len(orch.risk_state.mirofish_signal_runs_london) == 2
...
def test_stale_close_requests_expire_after_ttl():
    ...
    orch._mirofish_close_requested["pos-1"] = now - timedelta(seconds=200)
    orch._mirofish_close_requested["pos-2"] = now

    orch._clear_stale_close_requests(open_position_ids={"pos-1", "pos-2"}, now_utc=now + timedelta(seconds=1))

    assert "pos-1" not in orch._mirofish_close_requested
    assert orch._mirofish_close_requested["pos-2"] >= now
...
def test_live_timers_catch_up_after_restarts_and_run_after_force_flat():
    ...
    bridge_timer = _read_ops_file("mirofish-bridge.timer")
    ...
    assert "Persistent=true" in bridge_timer
    assert "OnCalendar=Mon-Fri *-*-* 08:00:00 Europe/London" in bridge_timer
    assert "OnCalendar=Mon-Fri *-*-* 11:30:00 Europe/London" in bridge_timer
...
def test_oracle_dashboard_service_uses_repo_placeholders_instead_of_local_user():
    service = _read_ops_file("oracle-dashboard.service")
    script = _read_ops_file("run_oracle_dashboard.sh")
...
def test_bridge_service_runs_without_legacy_backend_requirement():
    service = _read_ops_file("mirofish-bridge.service")

    assert "After=network-online.target xauex.service" in service
    assert "Requires=xauex.service" in service
    assert "mirofish-backend.service" not in service

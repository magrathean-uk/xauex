...
from bridge.signal_parser import _fallback_direction, _normalize_signal
...
def test_confidence_scales_lot_size_without_skipping_trade():
    dummy = SimpleNamespace(
        config=SimpleNamespace(
            mirofish_confidence_full_threshold=0.65,
            mirofish_confidence_medium_threshold=0.55,
            mirofish_medium_confidence_lot_multiplier=0.5,
            mirofish_low_confidence_lot_multiplier=0.25,
            max_lot_size=1.0,
        ),
    ...
    dummy._mirofish_lot_multiplier = lambda confidence: BotOrchestrator._mirofish_lot_multiplier(dummy, confidence)

...
def _extract_signal(cmd: dict[str, Any]) -> dict[str, Any]:
    signal = cmd.get("xauex_signal") or {}
...
def _build_report_url(signal: dict[str, Any], brief_meta: dict[str, Any]) -> str | None:
    ...
    if not report_id:
        return None
    return f"{XAUEX_API_URL}/api/report/{report_id}/download"
...
def _trade_explanation(
    ...
    xauex_positions = [
        ...
        if str(position.get("owner", "") or "").lower() == "xauex"
    ]
    manual_positions = [
        position for position in open_positions
    ...
    if manual_positions:
        return "Manual positions are open, but XAUEX does not count them toward its own auto-entry limits."
    if action == "HOLD":
        return "No trade is open because the latest XAUEX decision is HOLD."
